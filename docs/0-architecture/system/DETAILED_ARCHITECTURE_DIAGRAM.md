...
<!-- AGENT_CONTEXT
Purpose: Comprehensive architecture reference for MAGNETV1 naval design system
Authoritative: Yes
Keywords: architecture, modules, api, validators, physics, hull_gen, webgl, state, phases
Entry_Points:
  - "Generate hull" → Section 5 (Kernel Layer) + Section 7 (Hull Generation)
  - "Run validation" → Section 9 (Validators)
  - "Export geometry" → Section 8 (WebGL Pipeline)
  - "Query state" → Section 4 (Core Layer)
  - "Execute phase" → Section 5 (Kernel Layer, Conductor.run_phase)
Depends_On: None (this is root reference)
Used_By: All agents, developers, onboarding
Status: current
Last_Verified: 2026-01-25
-->

## Comprehensive Codebase Analysis — January 25, 2026
...
|--------------|------------|-----------|
| Generate hull geometry | [§5 Kernel Layer](#5-kernel-layer) | `kernel/conductor.py`, `hull_gen/generator.py` |
| Run physics validation | [§9 Validators](#9-validators--taxonomy) | `physics/validators.py`, `validators/executor.py` |
| Export to WebGL/glTF | [§8 WebGL Pipeline](#8-webglvisualization-pipeline) | `webgl/exporter.py`, `webgl/geometry_pipeline.py` |
| Query design state | [§4 Core Layer](#4-core-layer) | `core/state_manager.py` |
| Propose geometry changes | [§11 Agents](#11-agents-layer) | `agents/geometry_proposer.py` |
| Debug validation failure | [§9 Validators](#9-validators--taxonomy) | `dependencies/invalidation.py` |
| Add new validator | [§9 Validators](#9-validators--taxonomy) | `validators/registry.py` |
| Understand error | [§9 Validators](#9-validators--taxonomy) | `errors/taxonomy.py` |

## By Module (Alphabetical)
| Module | Purpose | Key Section |
|--------|---------|-------------|
| `agents/` | LLM-powered design agents | [§11 Agents Layer](#11-agents-layer) |
| `core/` | State management, constants | [§4 Core Layer](#4-core-layer) |
| `dependencies/` | Invalidation cascade | [§9 Validators](#9-validators--taxonomy) |
| `hull_gen/` | Hull geometry generation | [§7 Hull Generation](#7-hull-generation-pipeline) |
| `kernel/` | Phase orchestration | [§5 Kernel Layer](#5-kernel-layer) |
| `physics/` | Hydrostatics, resistance | [§6 Physics Layer](#6-physics-layer) |
| `validators/` | Validation framework | [§9 Validators](#9-validators--taxonomy) |
| `webgl/` | 3D visualization | [§8 WebGL Pipeline](#8-webglvisualization-pipeline) |
...
| Metric | Count |
|--------|-------|
| **Total Python Files** | 766 |
| **magnet/ modules** | 502 files |
| **tests/ files** | 264 files |
| **Validator Implementations** | 29 (14 registered + 15 additional) |
| **API Endpoints** | 44 |
| **Design State Sections** | 27 |
| **Design Phases** | 13 |
| **Physical Constants** | 96 |

**Largest Modules:**
| Module | Size | Purpose |
|--------|------|---------|
| `deployment/api.py` | 3,795 lines | REST API (38 core endpoints) |
| `agents/geometry_proposer.py` | 2,364 lines | Hull geometry proposals |
| `deployment/spiral_endpoints.py` | 2,246 lines | Spiral framework API (8 endpoints) |
| `physics/validators.py` | 2,429 lines | Physics validation |
| `kernel/synthesis.py` | 2,273 lines | Hull generation engine |
| `hull_gen/parameters.py` | 2,075 lines | Hull definition parameters |
...
│   │
│   ├── deployment/                      (11 files)
│   │   ├── api.py                       — FastAPI routes (38 core endpoints)
│   │   ├── spiral_endpoints.py          — Spiral framework API (8 endpoints, 2,246 lines) [NEW]
│   │   ├── error_handlers.py            — Enhanced error handling (322 lines) [NEW]
...
│   │
│   ├── kernel/                          (37 files, includes stdlib/, priors/, validators/)
...
│   │   └── validators/                  (1 file)
│   │       └── planarity.py             — Planarity validation
│   │
│   ├── lifecycle/                       (4 files)
...
deployment/spiral_endpoints.py (NEW - Spiral Framework)
    ├── kernel/synthesis.py (hull shell detection)
    ├── core/state_manager.py
    ├── deployment/design_store.py
    ├── deployment/websocket.py (WS broadcasts)
    └── deployment/error_handlers.py (centralized error handling)
```

---

# 3.3 Recent Development Activity (Jan 25, 2026)

## Completed Tasks (Jan 17-25, 2026)
| Task | Description |
|------|-------------|
| **SPIRAL-001** | Spiral framework endpoints (spiral_endpoints.py) |
| **ERR-001** | Enhanced error handling module |
| **TEST-001** | Test coverage expansion (+68 test files) |
| **TASK-001** | Hull end-cap triangulation fix |
| **TASK-003** | Geometry-based synthesis migration path |
| **PHASE 0** | Foundation Lock - Coordinate conventions, tolerances, docs |

## New Components (Jan 2026)
| Component | Size | Description |
|-----------|------|-------------|
| `deployment/spiral_endpoints.py` | 2,246 lines | Spiral framework with 8 new API endpoints |
| `deployment/error_handlers.py` | 322 lines | Enhanced error handling utilities |

## Infrastructure Enhancements
- Spiral framework for design-scoped, persistent design iteration
- Context-less design operations support
- LLM-first intent compilation with fallback
- CI workflow enhanced with invariant and validation tests
- WebGL LWL property handling enhanced
- Test suite expanded (+35% coverage)

## New Spiral Endpoints
| Endpoint | Purpose |
|----------|---------|
| `GET /api/v1/designs/{id}/debug/last-edit` | Debug last edit operation |
| `GET /api/v1/designs/{id}/debug/last-chat` | Debug last chat response |
| `GET /api/v1/designs/{id}/shape-document` | Access shape document |
| `POST /api/v1/designs/{id}/approve_decision` | Approval decision handling |
| `POST /api/v1/designs/{id}/integrity/repair` | Integrity repair operation |
| `POST /api/v1/designs/{id}/spiral/chat` | Spiral chat conversation |
| `POST /api/v1/designs/{id}/spiral/sketch` | Spiral sketch interpretation |
| `POST /api/v1/designs/{id}/migrate-to-geometry` | Geometry migration |

## Key Recent Commits
| Commit | Description |
|--------|-------------|
| `bd03c22` | Refactor geometry handling in design endpoints |
| `cdf9375` | Enhance CI workflow with invariant/validation tests |
| `6b42dd4` | Design endpoints: error handling + context-less ops |
| `5d0d14c` | TASK-003: Geometry-based synthesis |
| `244d826` | TASK-001: Hull end-cap triangulation fix |
| `e1364f2` | PHASE 0: Foundation Lock |

## Other Infrastructure Components
- `kernel/validators/planarity.py` (6,054 bytes) — Planarity validation with warp > 0.02 → PlanarityGateError
- Enhanced CI pipeline with invariant tests
- Context-less design endpoint operations
- LLM-first intent compilation with fallback
...
**File:** `magnet/core/constants.py` (265 lines, 96 constants)
...
**File:** `magnet/kernel/conductor.py` (1,131 lines)
...
**hull_families.py (DEPRECATED, 372 lines):**
```python
...
**File:** `magnet/physics/polygon_ops.py` (204 lines)
...
## 9.2 Registered Validators (14 in Registry)

| Validator ID | Class | File | Category | Phase | Severity |
|--------------|-------|------|----------|-------|----------|
| `physics/hydrostatics` | `HydrostaticsValidator` | `physics/validators.py` | PHYSICS | hull | **CRITICAL (GATE)** |
| `physics/resistance` | `ResistanceValidator` | `physics/validators.py` | PHYSICS | hull | NORMAL |
| `physics/equilibrium_draft` | `EquilibriumDraftValidator` | `physics/validators.py` | PHYSICS | weight | HIGH |
| `stability/intact_gm` | `IntactGMValidator` | `stability/validators.py` | STABILITY | stability | **CRITICAL** |
| `stability/gz_curve` | `GZCurveValidator` | `stability/validators.py` | STABILITY | stability | **CRITICAL** |
| `stability/damage` | `DamageStabilityValidator` | `stability/validators.py` | STABILITY | stability | HIGH |
| `stability/weather_criterion` | `WeatherCriterionValidator` | `stability/validators.py` | STABILITY | stability | HIGH |
| `weight/estimation` | `WeightEstimationValidator` | `weight/validators.py` | WEIGHT | weight | **CRITICAL** |
| `compliance/regulatory` | `ComplianceValidator` | `compliance/validators.py` | CLASS_RULES | compliance | **CRITICAL (GATE)** |
| `arrangement/generator` | `ArrangementValidator` | `arrangement/validators.py` | ARRANGEMENT | arrangement | NORMAL |
| `loading/computer` | `LoadingComputerValidator` | `loading/validators.py` | LOADING | loading | HIGH |
| `production/planning` | `ProductionPlanningValidator` | `production/validators.py` | PRODUCTION | production | NORMAL |
| `cost/estimation` | `CostValidator` | `cost/validators.py` | ECONOMICS | cost | NORMAL |

## 9.2.1 Additional Validators (Outside Registry)

| Class | File | Purpose |
|-------|------|---------|
| `NoiseVibrationValidator` | `analysis/noise_vibration_validator.py` | Noise/vibration analysis |
| `SeakeepingValidator` | `analysis/seakeeping_validator.py` | Seakeeping analysis |
| `ActionPlanValidator` | `kernel/action_validator.py` | LLM firewall |
| `DeckEquipmentValidator` | `outfitting/deck_equipment_validator.py` | Deck equipment |
| `OutfittingValidator` | `outfitting/validator.py` | Outfitting validation |
| `EnvelopeValidator` | `performance/envelope_validator.py` | Performance envelope |
| `PerformanceValidator` | `performance/predictor_validator.py` | Performance prediction |
| `CoatingsValidator` | `production/coatings_validator.py` | Coatings validation |
| `RoutingValidator` | `routing/agent/validators.py` | Routing validation |
| `ElectricalValidator` | `systems/electrical/validator.py` | Electrical systems |
| `FuelValidator` | `systems/fuel/validator.py` | Fuel systems |
| `HVACValidator` | `systems/hvac/validator.py` | HVAC systems |
| `PropulsionValidator` | `systems/propulsion/validator.py` | Propulsion systems |
| `SafetyValidator` | `systems/safety/validator.py` | Safety systems |
| `WeightSummaryValidator` | `weight/summary_validator.py` | Weight summary |

**Total Validator Implementations: 29** (14 registered + 15 additional)
...
**Files:**
- `magnet/deployment/api.py` (3,795 lines, 38 core endpoints)
- `magnet/deployment/spiral_endpoints.py` (2,246 lines, 8 Spiral endpoints)
...
### Spiral Framework (8 endpoints) — NEW
**File:** `magnet/deployment/spiral_endpoints.py`

| Method | Endpoint | Purpose |
|--------|----------|---------|
| GET | `/api/v1/designs/{design_id}/debug/last-edit` | Debug: last edit operation |
| GET | `/api/v1/designs/{design_id}/debug/last-chat` | Debug: last LLM chat response |
| GET | `/api/v1/designs/{design_id}/shape-document` | Access shape document state |
| POST | `/api/v1/designs/{design_id}/approve_decision` | Handle approval decisions |
| POST | `/api/v1/designs/{design_id}/integrity/repair` | Repair state integrity issues |
| POST | `/api/v1/designs/{design_id}/spiral/chat` | Spiral chat conversation |
| POST | `/api/v1/designs/{design_id}/spiral/sketch` | Spiral sketch interpretation |
| POST | `/api/v1/designs/{design_id}/migrate-to-geometry` | Migrate to geometry-based system |
...
# File counts
find magnet -name "*.py" | wc -l    # Expected: 457
find tests -name "*.py" | wc -l     # Expected: 196
```
...
## Hull Generation Path Selection
```
IF state.design_program exists
  → Use Conductor._run_program_generation() [NEW PATH]
  → Files: kernel/stdlib/*, kernel/program_executor.py
ELSE IF HullFamily enum is set (DEPRECATED)
  → Use Conductor._run_hull_synthesis() [LEGACY PATH]
  → Files: kernel/priors/hull_families.py, kernel/synthesis.py
ELSE (preferred for new designs)
  → Use get_defaults_from_froude(froude_number)
  → Files: kernel/priors/geometry_defaults.py
```

## Resistance Method Selection
```
IF Fn < 0.40
  → Pure Holtrop (weight = 1.0)
  → File: physics/resistance.py
ELSE IF 0.40 ≤ Fn < 0.70
  → Sigmoid blending (w_sav = 1/(1+exp(-k*(Fn-0.55))))
  → File: physics/validators.py:ResistanceValidator
ELSE IF Fn ≥ 0.70
  → Pure Savitsky (weight = 1.0)
  → File: physics/savitsky.py
```

## Validator Execution Path
```
IF validator_id in ValidatorRegistry
  → Run via PipelineExecutor.execute()
  → File: validators/executor.py
ELSE IF validator is "additional" (15 unregistered)
  → Import directly and call .validate()
  → Files: See §9 Additional Validators table
```
...
| Error Category | Code | Detection Point | Recovery Strategy | Next Action |
|----------------|------|-----------------|-------------------|-------------|
| `BND_EXCEEDED` | 2001-2007 | `parameter_bounds.py` | Clamp to bounds | Re-run validator |
| `PHY_IMPOSSIBLE` | 4001-4007 | `physics/validators.py` | None (fatal) | Report to user |
| `PHY_CONVERGENCE` | 4005 | `physics/equilibrium.py` | Adjust draft guess | Re-run with bisection |
| `STA_TRANSACTION` | 7001-7005 | `transactions/manager.py` | Rollback | Retry with new version |
| `CON_CONFLICTING` | 5001-5005 | `dependencies/graph.py` | Invalidate stale | Re-run cascade |
| `PlanarityGateError` | — | `kernel/validators/planarity.py` | Reduce panel warp | Regenerate sections |

## Error Factory Quick Reference
```python
from magnet.errors.taxonomy import make_bounds_error, make_physics_error
make_bounds_error(code=2001, message="LOA exceeds 500m", path="hull.loa")
make_physics_error(code=4003, message="Negative displacement")
```
...
How an LLM agent should use these indexes:

## Step 1: Identify Task Type
```
USER REQUEST → classify as one of:
  - "generate" (hull, geometry, mesh)
  - "validate" (physics, compliance, constraints)
  - "query" (read state, get values)
  - "modify" (propose changes, update parameters)
  - "export" (WebGL, glTF, 2D drawings)
  - "debug" (understand error, trace failure)
```

## Step 2: Consult Quick Navigation Index
```
TASK TYPE → look up in "By Task" table → get:
  - Section reference (e.g., §5 Kernel Layer)
  - Key files (e.g., kernel/conductor.py)
```

## Step 3: Check Decision Tree (If Multiple Paths)
```
IF task has multiple implementation paths:
  → Read Decision Trees section
  → Evaluate conditions (design_program exists? Fn value?)
  → Select correct path
```
...
IF error encountered:
  → Look up error code/category in Error Recovery Matrix
  → Apply recovery strategy
  → Execute next action
```

## Step 5: Verify via Code
```
ALWAYS:
  → Read the actual file before making changes
  → Check docstrings for current interface contracts
  → Use grep to find `state_manager.set()` calls for mutation paths
```
...
| **Total Python Files** | 766 |
| **Validator Count** | 29 (14 registered + 15 additional) |
| **API Endpoints** | 44 |
| **Design State Sections** | 27 |
| **Design Phases** | 13 |
| **Physical Constants** | 96 |
